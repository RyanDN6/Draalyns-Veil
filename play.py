...
try:
    ...
    def generateEnemies(tile):
        ...
        choices = [enemy for enemy in choices if power < 3]
    ...
    def playMap(matrix, player_pos, area, party, scroll=None):
        ...
        steps = 0    
        mixer.music.load("sounds\Further.mp3")
        mixer.music.play(100)    
        ...
        try:
            while True:
                ...
                screen.addstr(1, 1, f" Area: {area} | Gold {party[0].gold}", curses.color_pair(3))
                # Draw top border
                ...
                for i, row in enumerate(matrix):
                    ...
                    for j, char in enumerate(row):
                        ...
                        if (j, i) == (x, y):
                            screen.addch(i + 3, j * 2 + 1, '@')  # Player position
                        else:
                ...
                if key in ["w", "a", "s", "d"]:
                    ...
                    if matrix[new_y][new_x] in ['.','^', '%', '|', '~']:
                        ...
                        if matrix[new_y][new_x] == "|":
                            ...
                            scroll.text(f"Entering {t.names[area]}...", slow=True)
                            mixer.music.load("sounds\Clouds.mp3")
                            mixer.music.play(0, 0, 4000)
                            
                            t.enter(area, party, first=True)
                            
                            mixer.music.fadeout(2000)
                            mixer.music.load("sounds\Further.mp3")
                            mixer.music.play(100, 0, 1000)
                            steps = 0
